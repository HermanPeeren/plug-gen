...
namespace administrator\components\com_pluggen\src\Service\HTML;
...
use Yepr\Component\Pluggen\Site\Helper\RouteHelper;
...
class Icon
{
    ...
    public static function create($category, $params, $attribs = [])
    {
        ...
        $url = 'index.php?option=com_pluggen&task=project.add&return=' . base64_encode($uri) . '&id=0&catid=' . $category->id;
        ...
        $output = '<span class="hasTooltip" title="' . HTMLHelper::_('tooltipText', 'COM_PLUGGEN_CREATE_PROJECT') . '">' . $button . '</span>';
    ...
    public static function edit($project, $params, $attribs = [], $legacy = false)
    {
        ...
        if (property_exists($project, 'checked_out')
            ...
            $tooltip      = Text::_('JLIB_HTML_CHECKED_OUT') . ' :: ' . Text::sprintf('COM_PLUGGEN_CHECKED_OUT_BY', $checkoutUser->name)
                . ' <br /> ' . $date;
        ...
        $overlib .= $date;
        $overlib .= '&lt;br /&gt;';
        $overlib .= Text::sprintf('COM_PLUGGEN_WRITTEN_BY', htmlspecialchars($author, ENT_COMPAT, 'UTF-8'));
        ...
        $text = '<span class="hasTooltip fa fa-' . $icon . '" title="'
            . HTMLHelper::tooltipText(Text::_('COM_PLUGGEN_EDIT_PROJECT'), $overlib, 0, 0) . '"></span> ';
        $text .= Text::_('JGLOBAL_EDIT');

        $attribs['title'] = Text::_('COM_PLUGGEN_EDIT_PROJECT');
