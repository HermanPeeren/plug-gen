...
use DOMDocument;
use const Yepr\Component\Pluggen\Administrator\Model\Generator\JPATH_ROOT;

/**
 * A concrete generator to create the project-form files
 * generated files: form xml-files
 *
 * @package     Yepr\Component\Pluggen\Administrator\Model\Generator\
 *
 * @since       version 1.0
 */
class ProjectForms
{
	...
	public function __construct(string $projectFormName, object $AST, LanguageStringUtil $languageStringUtil)
	{
		...
		$this->projectFormNamespace = 'Yepr\\Component\\Pluggen\\Administrator\\Projectform\\';
	}
	...
	public function generate(): array
	{
		...
		$generatedProjectFormFilesPath = $this->pluggenAdminPath . 'forms/ProjectForms/' . $this->projectFormName .'/';
	...
	/*	$treeIterate($root, 'createForm')
		...
		// todo: make this more general to use it when building generators in Pluggen
		$entityMap = [];
		$fieldMap  = [];
		...
		$generatedFilesPathComponent = $pluggenAdminPath . '/forms/projectForms/' . $this->projectFormName .'/';
