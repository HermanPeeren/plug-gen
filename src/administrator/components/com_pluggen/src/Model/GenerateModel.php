...
use	administrator\components\com_pluggen\src\Model\LanguageStringUtil;
use const Yepr\Component\Pluggen\Administrator\Model\JPATH_ROOT;
...
class GenerateModel extends AdminModel
{
	...
	public function generate()
	{
		...
		$outputType = "Joomla6"; // todo: get the outputType from the generator... multiple generators...
		$AST = $this->initiateAST();
		$languageStringUtil = new LanguageStringUtil($AST);

		// Plugin name
		$plugin = $AST->extensions->component;
		$pluginName = $plugin->plugin_name;

		// file paths for generated (language)files
		$pluggenAdminPath = JPATH_ROOT . '/administrator/plugins/com_pluggen/';
		$generatedFilesPathPlugin = $pluggenAdminPath . '/generated/' . $pluginName .'/'
			. $outputType . '/com_'.strtolower($pluginName) . '/';

		foreach ($this->outputTypes as $outputType)
		{
			$generatorNamespace = 'Yepr\\Component\\Pluggen\\Administrator\\Model\\Generator\\' . $outputType . '\\';

			// --- Component ---
			$this->log[] = "<b>=== PLUGIN GENERAL ===</b>";
			$this->useConcreteGenerator($generatorNamespace . "PluginGeneral", $outputType, $AST, $languageStringUtil);
	 ...
	 * @param   string              $outputType           The type of output we generate files for, for instance "Joomla6"
	...
	private function initiateAST(): object
	{
		...
		$getASTquery = $db->getQuery(true)
			...
			->from($db->quoteName('#__pluggen_projects'))
