...
namespace administrator\components\com_pluggen\src\View\Project;
...
class HtmlView extends BaseHtmlView
{
	...
	protected function addToolbar()
	{
		...
		ToolbarHelper::title($isNew ? Text::_('COM_PLUGGEN_MANAGER_PROJECT_NEW') : Text::_('COM_PLUGGEN_MANAGER_PROJECT_EDIT'), 'project');

		// Since we don't track these assets at the item level, use the category id.
		$canDo = ContentHelper::getActions('com_pluggen'); //, 'category', $this->item->catid

		// Build the actions for new and existing records.
		if ($isNew)
		{
			// For new records, check the create permission.
			//if ($isNew && (count($user->getAuthorisedCategories('com_pluggen', 'core.create')) > 0))
			//{
