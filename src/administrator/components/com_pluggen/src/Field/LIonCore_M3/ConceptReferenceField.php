...
namespace administrator\components\com_pluggen\src\Field\LIonCore_M3;
...
class ConceptReferenceField extends ListField
{
	//The field class must know its own type through the variable $type.
	protected $type = 'ConceptReference';

	/**
	 * Get the options for the list field: all defined interfaces that can be implemented + the (abstract) classifier.
	 * To be used as a single field to indicate from what other ConceptInterface was extended (or from the abstract classifier)
	 *
	 */
	public function getOptions()
	{
		...
		if (!is_null($AST))
		{
			...
			foreach ($AST->languageEntities as $languageEntity)
			{
				...
				if (($languageEntity->languageEntity_type == 'Classifier')
					&& ($languageEntity->classifier->classifier_type == 'Concept'))
				{
		...
		// Todo: sort
		// Todo: also get the Concepts of the dependent languages, if applicable
	...
	private function initiateAST(): ?object
	{
		...
		if ($projectFormId)
		{
			...
			$getASTquery = $db->getQuery(true)
				...
				->from($db->quoteName('#__pluggen_projectforms'))
